#!/usr/bin/env python3
"""Evaluate base RelCompat3D scores on the development or validation split."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--role", choices=("development", "final_validation"), required=True)
...
def controls(grouped: dict[str, list[dict[str, Any]]], gt: dict[str, set[tuple[Any, ...]]], models: dict[str, Any], subgraphs: list[str], samples: np.ndarray) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "support_contact_endpoint_swap": {"status": "not_applicable"},
    }
...
def main() -> int:
    ...
    if args.model_selection:
        paths["model_selection"] = resolve(root, args.model_selection)
    ...
    if models.get("schema_version") != "relcompat3d_base_models_v1":
        raise ValueError("base_models_not_ready")
    if args.role == "final_validation":
        if "model_selection" not in paths:
            raise ValueError("final_validation_requires_model_selection")
        selection = json.loads(paths["model_selection"].read_text(encoding="utf-8"))
        if selection.get("status") != "selected":
            raise ValueError("model_selection_not_recorded")
    ...
    expected_seed = 20260711 if args.role == "development" else 20260712
    validations = {
        ...
        "bootstrap_configuration_matches": args.n_bootstrap == 1000 and args.seed == expected_seed,
        "validation_model_selection_recorded": args.role != "final_validation" or "model_selection" in paths,
    }
    report = {
        "schema_version": "relcompat3d_base_model_source_evaluation_v1",
