#!/usr/bin/env python3
"""Evaluate trained RelCompat3D models on locally prepared prediction rows."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--linear-models", type=Path, required=True)
    parser.add_argument("--mlp-models", type=Path, required=True)
...
def main() -> int:
    ...
    inputs = {
        ...
        "linear_models": args.linear_models,
        "mlp_models": args.mlp_models,
        "vlsat": args.vlsat,
    ...
    gt, _ = model_eval.load_gt(args.ground_truth)
    linear_models = json.loads(args.linear_models.read_text(encoding="utf-8"))
    mlp_models = json.loads(args.mlp_models.read_text(encoding="utf-8"))
    scorer = linear.make_linear_scorer(linear_models)
    bce_model = mlp_models["shared_mlp_bce"]
    pairwise_model = mlp_models["shared_mlp_pairwise"]
    ...
    for index, (source, path) in enumerate(source_paths.items()):
        sources[source], ranking_checks[source] = comparators.evaluate_source(
            path,
            args.ground_truth,
            contexts,
            scorer,
    ...
    validations = {
        ...
        "family_composition_preserved": all(
            value["family_composition_exact"] for value in ranking_checks.values()
        ),
        "support_contact_selection_preserved": all(
            value["support_selection_exact"] for value in ranking_checks.values()
        ),
        "models_exclude_source_inputs": (
            linear_models.get("source_score_used") is False
            and linear_models.get("source_identity_used") is False
            and not pairwise_model["feature_spec"]["source_score_input"]
            and not pairwise_model["feature_spec"]["source_identity_input"]
        ),
    }
    status = "completed" if all(validations.values()) else "failed_validation"
    summary = {
        "schema_version": "relcompat3d_evaluation_v1",
        "status": status,
    ...
    manifest = {
        "schema_version": "relcompat3d_evaluation_manifest_v1",
        "status": status,
        "inputs": {
